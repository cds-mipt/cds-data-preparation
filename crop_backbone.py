...
def save(image, id, unique_id, filename):
    ...
    if None not in image and image.shape[2] == 3 and image.shape[1] > 5 and image.shape[0] > 5:
        image = cv2.resize(image, (71, 71))
        if image.shape != (71, 71, 3) or None in image:
            print("WTF")
            return 0
        image = np.array([np.array([i for i in j])for j in image])
        cv2.imwrite(args.output_folder_train + "/"+str(jjj) + ".jpg", np.array(image))
        all_images.append(image)
        annotations.append([id0(id[0]), id4(id[4]), id3(id[3])])
        jjj += 1
...
def main(args):
    ...
    n = int(len(all_images)*0.8)
    with open(args.output_folder_train + "/" + "all_images", 'wb') as f:
        pickle.dump(all_images[:n], f)
    with open(args.output_folder_train + "/" + "annotations", 'wb') as f:
        pickle.dump(annotations[:n], f)


    with open(args.output_folder_train + "/" + "all_images_test", 'wb') as f:
        pickle.dump(all_images[n:], f)
    with open(args.output_folder_train + "/" + "annotations_test", 'wb') as f:
        pickle.dump(annotations[n:], f)
